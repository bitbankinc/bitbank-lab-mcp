...
const ClosedPositionPnlSchema = z.object({
	...
	realized_pnl_unavailable_reason: PortfolioCostBasisUnavailableReasonEnum.optional().describe(
		'realized_pnl を算出できなかった理由（#93）。設定されている場合 realized_pnl / priced_deposit_count / unpriced_deposit_count はいずれも undefined——原価計算そのものを試みていない。現状は untracked_trade_suspected のみが該当し、暗号資産の DONE 入庫はあるが約定履歴にも現在残高にも現れない銘柄（入出金履歴だけから検出した銘柄）で立つ。**holdings[].cost_basis_unavailable_reason と同じ enum を共有するが意味は異なる**——あちらは「原価計算を試みたが確定できなかった」、こちらは「原価計算の入力（約定履歴）自体が無く、そもそも試みていない」。本フィールドが立つ要素は closed_position_realized_pnl / closed_position_asset_count の集計に含まれない（realized_pnl を持たないため和の対象外）。**closedSuppressed（入庫日価格を解決できない売り切り銘柄が 1 件でもあると closed_position_realized_pnl 自体を undefined にする既存の抑止、#92）とは独立に動く**——closed_position_realized_pnl / closed_position_asset_count が undefined の実行でも、本フィールドが立つ要素は closed_positions 配列に残る。**断定はできない**——「入庫はあるが痕跡が消えている」状態は、他の要因（例: 履歴に現れない出庫）でも起こりうる。**この検出にも限界がある**——入庫そのものが無く、取引所約定も無く、販売所（即時売買）のみで売買を完結させた銘柄は入出金履歴にも約定履歴にも痕跡が残らないため検出できない',
	),
});
