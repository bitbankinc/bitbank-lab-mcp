...
const EquityPointSchema = z.object({
	...
	flow_jpy: z
		...
		.optional()
		.describe(
			'この点から**次の点まで**の区間に発生した純入出金（元本移動のみ、円）。正 = 純入金、負 = 純出金。value_jpy は「その時点で入出金を巻き戻した」値なので、この点の入出金が評価額に現れるのは次の点から: value_jpy[i+1] - value_jpy[i] - flow_jpy[i] = 区間 i の市場変動（最終点も同式に載る）。この向きにより timestamp は入出金の発生日（月次点なら発生月）そのものを指す。出金手数料は含まない（*_performance.net_flow_jpy と同一定義。手数料コストは上式の残差に市場変動と一緒に残る）。JPY の入出金と、JPY 換算できた暗号資産入出庫の合計。換算は入出庫日の始値を優先し、**その日の価格を解決できなかった分は現在価格にフォールバックしたうえで計上する**（本値は全額が入出庫日で固定された評価額とは限らない。フォールバック件数は meta.flowValuationFallbackCount / meta.flowValuationBasis と summary 先頭の「n 件は現在価格で仮評価」が申告する）。どちらでも価格を解決できなかった入出庫は計上せず、資産名は同じ期間を張る *_performance.unpriced_flow_assets（monthly_equity_series ↔ monthly_performance、yearly_equity_series ↔ yearly_performance）で申告される。同じ区間の入金と出金は純額で相殺するため、**キーの不在は「入出金が無かった」ではなく「純額がゼロ」**を意味する。最終点（現在のリアルタイム評価額）は次の点が無いため常に undefined。入出金履歴が欠けている構成（取得失敗 / 一部チャネル失敗 / 件数上限による打ち切り）では部分集合の合計を確定値として出さず全点で undefined になり、その状態は *_performance.flow_measured=false / flow_unavailable_reason で申告される',
		),
});
