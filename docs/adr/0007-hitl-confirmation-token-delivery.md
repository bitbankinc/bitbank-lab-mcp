...
| 使用済み token の replay（プロセス再起動 / 複数プロセス） | 該当なし | △ HMAC は永続 secret、使用済み記録はプロセス内メモリのみ。TTL 内の再起動で 1 回だけ通る。同上 |

## ワンタイム性の保証範囲（2026-08-14 追記）

`validateToken` のワンショット制約は、**「サーバープロセスの壁時計が単調に進む」かつ
「プロセスが生存し続ける」場合にのみ成立する。** どちらも仕様ではなく前提であり、崩れると
同じトークンが 2 回通る。被害の上限は上表と変わらない（プレビュー済みの注文 1 件、または
プレビュー済み ID 集合 1 セットが、もう 1 回）。任意の注文は依然として作れない。
...
（`lib/bounded-expiring-set.ts` の `has` / `purgeExpired` / 定期 purge タイマ）。したがって:

- **前方ジャンプ**: TTL の途中で時計が `expiresAt` を超えて進むと、生存中の使用済み記録が
  期限切れとみなされて消える。時計が戻れば `validateToken` は「未使用」と判断する
- **後方ジャンプ**: TTL が正常に経過して記録が消えたあと、壁時計を `expiresAt` 以前へ戻すと
  期限チェックが再び通る。**境界の直後なら数 ms の後方ステップで足りる**

**修正しない理由**: 攻撃者はサーバー機の時計を誘発できず、トークン保持者は iframe /
ユーザー本人である。検討した緩和策（記録の削除判定だけを単調時計で行う）は前方ジャンプしか
...
- TTL（既定 60 秒）内にサーバープロセスが再起動すると、使用済みトークンがもう一度通る
- 同じ secret を持つ**別プロセスでも通る**。Claude Desktop は surface ごとにサーバーを
  spawn することがあり、同一マシン上で複数プロセスが同時に走る構成を実際に確認している
...
iframe しか保持していない。ただし**引き金（プロセス再起動）の頻度は時計異常より桁違いに高い。**
