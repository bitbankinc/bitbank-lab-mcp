...
- **Updated**:
  ...
  - 2026-08-17（**文書のみ**。Decision は変更しない）: 計測 4 を追加。`ui.resourceUri` を持たない
    ...
    「配信しない」という断定を訂正した
  - 2026-08-17（**実装変更**）: 確認トークンの署名鍵を `BITBANK_API_SECRET` 単独から
    `BITBANK_API_SECRET` + per-process nonce へ変更。ワンショット性の保証範囲（プロセス内）と
    トークンの有効範囲を一致させ、再起動 / 別プロセスでの replay を閉じた
    （「ワンタイム性の保証範囲」項目 2）。Decision 自体は変更しない
...
| ワンタイム | `validateToken` が `BoundedExpiringSet` に登録できた場合のみ成功（fail-closed） | 変更なし。ただし**保証範囲には前提がある**（壁時計の単調性）。下記「ワンタイム性の保証範囲」を必ず参照 |
| トークンのプロセス束縛 | 署名鍵 = `BITBANK_API_SECRET` + per-process nonce（`confirmation.ts` の `getSigningKey`） | **2026-08-17 追加**。再起動後・別プロセスでは `token_invalid`。ワンショット性の保証範囲とトークンの有効範囲を一致させる |
...
> **「1 回だけ」には前提がある。** 壁時計が単調に進むことを仮定しており、外すと回数は増える
> （下記「ワンタイム性の保証範囲」）。プロセス境界は 2026-08-17 に閉じた（署名鍵に
> per-process nonce）。**内容の限定（プレビュー済み 1 件 / ID 集合 1 セット）は条件によらず
> 維持される**——そちらは HMAC 束縛が担保しているため。
...
| token 漏洩時の最大被害（内容） | 該当なし | 直前にプレビューした注文 1 件（`cancel_orders` ではプレビュー済み ID 集合 1 セット）× 60 秒。HMAC 束縛なので条件によらない |
| token 漏洩時の最大被害（回数） | 該当なし | △ 壁時計が単調なら 1 回。**時計の非単調性を許すと増える**。プロセス境界は 2026-08-17 に閉じた。下記「ワンタイム性の保証範囲」 |
...
`validateToken` のワンショット制約は、**「サーバープロセスの壁時計が単調に進む」場合にのみ
成立する。** これは仕様ではなく前提であり、崩れると同じトークンが 2 回以上通る。

**プロセス境界は 2026-08-17 に閉じた。** 署名鍵に per-process nonce を混ぜたので、
再起動後・別プロセスでは旧トークンが `token_invalid` になる（項目 2）。
それ以前は「同時に走るプロセス数 + TTL 内の再起動回数」に比例して通っていた。

**残っているのは時計だけである。** 同一プロセス内で「使用済み記録が purge されたあとに
壁時計が巻き戻る」と再実行が通る（項目 1）。

**いずれの場合も「何を」実行できるかは変わらない。** 対象はプレビュー済みの注文 1 件、
または `preview_cancel_orders` でプレビューした ID 集合 1 セットに HMAC 束縛されており、
任意の注文は作れない。金額・数量・ペア・方向を攻撃者が選ぶこともできない。

> **上表および「既存の防御は 1 つも外さない」節の「60 秒 × 1 回」は、壁時計が単調である
> 場合の値である。** 時計の非単調性を許すと回数は増える。内容（プレビュー済み 1 件）の
> 限定は条件によらず維持される。

2026-08-14 のレビューで 3 点を検討した。**項目 2 は 2026-08-17 に修正済み、項目 1 と 3 は
修正しない**（理由は各項に書く）。ここに記すのは「知らずに踏まない」ためであって、
安全だと主張するためではない。

### 1. 壁時計の非単調性

`BoundedExpiringSet` は使用済み記録を `expiresAt`（壁時計の epoch ms）で消す
（`lib/bounded-expiring-set.ts` の `has` / `purgeExpired` / 定期 purge タイマ）。
したがって、次の 2 経路で記録が失われる。

- **前方ジャンプ**: TTL の途中で時計が `expiresAt` を超えて進み、**その状態で purge が走ると**
  生存中の使用済み記録が期限切れとみなされて消える。時計が戻れば `validateToken` は
  「未使用」と判断する。
  purge を走らせるのは 60 秒周期の cleanup タイマ（`startCleanupTimer`）、他キーの `add` が
  呼ぶ先頭走査、明示的な `purgeExpired` である。**`validateToken` 自身は削除しない** —
  期限チェック（`confirmation.ts:228`）が `usedTokens.has`（同 233）より先にあり、
  前方ジャンプ直後の検証は `token_expired` で戻るため `has` に到達しない
- **後方ジャンプ**: TTL が正常に経過して記録が消えたあと、壁時計を `expiresAt` 以前へ戻すと
  期限チェックが再び通る。**境界の直後なら数 ms の後方ステップで足りる**
...
塞がず、後方ジャンプは数 ms のステップで通るため実質的な緩和にならない。加えて
`performance.now()` のサスペンド中の挙動は OS 依存で（Linux の `CLOCK_MONOTONIC` は
サスペンド時間を数えない）、記録が壁時計 TTL を超えて残り容量 fail-closed に近づく。
replay ガードの中核に 2 つの時計を混ぜる実装リスク（epoch と `timeOrigin` 相対値の取り違えは
生存記録の全消しに直結する）に見合わない。

### 2. プロセス再起動 / 複数プロセス — **2026-08-17 に修正済み**

**かつての問題**: トークンの HMAC は `BITBANK_API_SECRET`（永続）だけで計算される一方、
使用済み記録はプロセス内メモリにしか無い（`src/private/confirmation.ts` の `usedTokens`）。
**ワンショット性はプロセス内に閉じているのに、トークンはプロセスに束縛されていない**という
非対称があり、同じトークンが受け取ったプロセスそれぞれで 1 回ずつ通った。

- **同時に走るプロセス数**: 同じ secret を持つ別プロセスも検証を通す。Claude Desktop は
  surface ごとにサーバーを spawn することがあり、同一マシン上で複数プロセスが同時に走る
  構成を実際に確認している
- **TTL 内の再起動回数**: 再起動のたびに `usedTokens` は空になり、記録はリセットされる

`requestState` 側にこの穴は無かった。per-process のランダム鍵を使っており、再起動で pending の
確認が無効化される（`src/private/request-state.ts` 冒頭）。**確認トークンだけが非対称だった。**

**修正**: 署名鍵を `BITBANK_API_SECRET` 単独から **`BITBANK_API_SECRET` + per-process nonce** に
変えた（`getSigningKey` / `getProcessNonce`）。nonce はプロセス起動ごとのランダム値で、
`globalThis` に置く——守りたい単位が「OS プロセス」であってモジュールインスタンスではないため。
これで**ワンショット性の保証範囲とトークンの有効範囲が一致する。**

- 再起動後は旧トークンが `token_invalid`（使用済み・未使用を問わず）
- 同じ secret を持つ別プロセスは互いのトークンを検証できない
- 同一プロセス内の挙動は変わらない（再利用は従来どおり `token_already_used`）

**代償**: プロセス再起動で**未使用の preview も無効化される**。これは経路 1（elicitation）の
pending 確認が再起動で失効するのと同じで、fail-closed 側に倒れる。ユーザーには
「preview からやり直してください」と見える。

回帰は `tests/private/confirmation.test.ts`（再起動後の使用済み / 未使用トークン、
再発行トークンの正常系、別プロセス相当で同一 params でもトークンが一致しないこと）。

> **時計（項目 1）は依然として未修正。** 署名鍵をプロセスに束縛しても、
> **同一プロセス内**での「記録 purge 後の巻き戻し」は閉じない。項目 1 と 2 は別の穴である。

### 3. `getUiSnapshotMeta` の期限判定も壁時計

`src/ui-snapshot-cache.ts` の `getUiSnapshotMeta` は `metaExpiresAtMs` を過ぎると `_meta` を
返さないが、**entry 自体は消さない**（消すと `getUiSnapshot` によるプレビュー再描画まで
巻き添えで死ぬため）。壁時計が戻ると、期限切れトークンが再び配送されうる。

**これは配送であって認可ではない。** 実行可否は `validateToken` が最後の門なので、項目 1 が
成立していない限り実害にはならない。単独では増幅器としてのみ効く。

### 未修正の項目にテストを足さないこと

項目 1 と 3 は**現に修正していない**。回帰テストを足すと「直っていないバグのテスト」になり、
CI が保証していない性質を保証しているように見せてしまう。保証範囲の境界を示しているのは
既存の `tests/private/confirmation.test.ts`（期限切れ後は `token_expired`）と
`tests/lib/bounded-expiring-set.test.ts`（ワンタイム性は TTL 内）である。

**修正した項目にはテストを足す。** 項目 2（プロセス束縛）は
`tests/private/confirmation.test.ts` に回帰を張ってある——**閉じた範囲だけ**を固定しており、
時計の巻き戻し（項目 1）は対象外である。
