...
2. **クライアントが MCP Apps UI を宣言していること** — `initialize` の client capabilities に
   `extensions["io.modelcontextprotocol/ui"]` が存在すること。判定は `clientSupportsElicitation`
   （`src/private/elicitation.ts`）と同じ要領で、`server.getClientCapabilities()` と
   per-request envelope（`extra.mcpReq.envelope.clientCapabilities`）の両方を見る。
   宣言が無いホストには**トークンを一切載せない**。
...
```text
ゲート（オプトイン AND MCP Apps UI 宣言）が偽        → direct_execute_forbidden（従来どおり）
...
これらにより、**トークンが漏れた場合の最悪ケースは「ユーザーが直前にプレビューしたその注文が、
60 秒以内に 1 回だけ実行される」**に限定される。任意の注文は作れず、金額・数量・ペア・方向を
攻撃者が選ぶこともできない。**この性質は本改訂で維持する。**
...
名前空間キーで返す。ゲートは preview 側とまったく同じ 2 段（オプトイン AND MCP Apps UI 宣言）を課す。
根拠は、これがチャネルを増やさず（同じ結果 `_meta`）、境界も増やさない（スナップショットは既に
`sessionId + resourceUri` 束縛・TTL 5 分・実行成功時に `clearUiSnapshot`）ため。トークン自体の
TTL 60 秒・ワンタイム・パラメータ束縛はそのまま効く。
...
| リスク | 既定（オプトイン off） | オプトイン on + MCP Apps UI 宣言ホスト |
|---|---|---|
...
| token 漏洩時の最大被害 | 該当なし | 直前にプレビューした注文 1 件 × 60 秒 × 1 回 |
...
| ホストが `extensions["io.modelcontextprotocol/ui"]` の宣言をやめる | ゲート 2 が閉じ、経路 3 に戻る（preview のみ） | 安全側 |
