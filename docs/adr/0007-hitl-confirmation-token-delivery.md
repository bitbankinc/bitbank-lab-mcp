...
| ワンタイム | `validateToken` が `BoundedExpiringSet` に登録できた場合のみ成功（fail-closed） | 変更なし。ただし**保証範囲には前提がある**（壁時計の単調性・プロセス生存）。下記「ワンタイム性の保証範囲」を必ず参照 |
...
> **「1 回だけ」には前提がある。** 使用済み記録はプロセス内メモリで、トークンはプロセスに
> 束縛されていない。単一プロセスかつ壁時計が単調、という条件を外すと回数は増える
> （下記「ワンタイム性の保証範囲」）。**内容の限定（プレビュー済み 1 件 / ID 集合 1 セット）は
> 条件によらず維持される**——そちらは HMAC 束縛が担保しているため。
...
> **2026-08-17 追記**: この節の「配信しない」という断定は不正確だった。計測 4 のとおり、
> 正しくは**「コールド接続時に 2.5 秒以内に間に合わないことがある」**（ウォーム 0 件 /
> コールド 1 件）。**判断そのものは有効**で、トークンを `get_ui_snapshot` から外すと
> Desktop 再起動後の 1 回目の発注が失敗する。またモデルへの漏洩懸念も計測 4 で否定された。
...
| token 漏洩時の最大被害（回数） | 該当なし | △ 単一プロセス・壁時計が単調なら 1 回。**条件が崩れると「プロセス数 + TTL 内の再起動回数」に比例**。下記「ワンタイム性の保証範囲」 |
...
| 使用済み token の replay（プロセス再起動 / 複数プロセス） | 該当なし | △ HMAC は永続 secret、使用済み記録はプロセス内メモリのみ。**受け取ったプロセスごとに 1 回ずつ**通る。同上 |

## ワンタイム性の保証範囲（2026-08-14 追記）

`validateToken` のワンショット制約は、**「サーバープロセスの壁時計が単調に進む」かつ
「トークンを検証しうるプロセスが 1 つだけ生存し続ける」場合にのみ成立する。**
どちらも仕様ではなく前提であり、崩れると同じトークンが 2 回以上通る。

**実行回数の上限は「1 回」ではない。** 使用済み記録はプロセス内メモリなので、同じ
`BITBANK_API_SECRET` を持つプロセスは互いの記録を知らない。TTL 内であれば
**プロセス 1 つにつき 1 回**通り、再起動のたびにリセットされる。したがって回数は
「同時に走るプロセス数 + TTL 内の再起動回数」に比例する（TTL 既定 60 秒がその窓）。

**上限が変わらないのは「何を」実行できるかの方である。** 対象は依然としてプレビュー済みの
注文 1 件、または `preview_cancel_orders` でプレビューした ID 集合 1 セットに HMAC 束縛
されており、任意の注文は作れない。金額・数量・ペア・方向を攻撃者が選ぶこともできない。

> **上表および「既存の防御は 1 つも外さない」節の「60 秒 × 1 回」は、この節の前提が
> 成立している場合の値である。** 単一プロセスかつ壁時計が単調、という条件を外すと
> 回数は上記のとおり増える。内容（プレビュー済み 1 件）の限定は条件によらず維持される。

2026-08-14 のレビューで 3 点を検討し、**いずれも本改訂では修正しない**と決めた。理由は各項に書く。
ここに記すのは「知らずに踏まない」ためであって、安全だと主張するためではない。
...
### 2. プロセス再起動 / 複数プロセス（時計より頻度が高い）

トークンの HMAC は `BITBANK_API_SECRET`（永続）で計算される一方、使用済み記録は
プロセス内メモリにしか無い（`src/private/confirmation.ts` の `usedTokens`）。つまり
**ワンショット性はプロセス内に閉じているのに、トークンはプロセスに束縛されていない。**

つまり**同じトークンは、それを受け取ったプロセスそれぞれで 1 回ずつ通る。**
TTL（既定 60 秒）が窓で、回数は次の合計に比例する。
...
`requestState` 側にこの穴は無い。per-process のランダム鍵を使っており、再起動で pending の
確認が無効化される（`src/private/request-state.ts` 冒頭）。**確認トークンだけが非対称。**

**修正しない理由**: 実行には「トークンを保持した第三者」が前提で、`_meta` 限定配送では
通常 iframe しか保持していない（漏洩・モデル可視ホストの場合は上表の依存点に戻る）。
ただし**引き金（プロセス再起動）の頻度は時計異常より桁違いに高い。**
塞ぐなら HMAC ペイロードに per-process 乱数を足す（`requestState` と同じ手法。未使用の
preview も再起動で無効化される＝経路 1 と同じ fail-closed 挙動になる）。
**時計の件より優先度が高い**ので、この領域のコードを触るならこちらから着手すること。
...
**これは配送であって認可ではない。** 実行可否は `validateToken` が最後の門なので、上の 1 / 2 が
同時に成立していない限り実害にはならない。単独では増幅器としてのみ効く。

### この節にテストを足さないこと

上の 3 点は**現に修正していない**。回帰テストを足すと「直っていないバグのテスト」になり、
...
**計測 1 だけでなく計測 4 も再実行すること**——トークンは preview 系（App ツール）と
`get_ui_snapshot`（通常ツール）の 2 経路で `_meta` に載るため、片方の計測では足りない。
