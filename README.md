...
* **API キーの置き場所で意味が変わります。** キーをクライアント側（`claude_desktop_config.json` の `env` 等）に設定している場合、クライアントは既にキーを保持しているため、このフラグによる追加のリスクはありません。**サーバー側の `.env` に置く構成では、クライアントに本来無い実行権限を与えることになります**。この構成での有効化は推奨しません
